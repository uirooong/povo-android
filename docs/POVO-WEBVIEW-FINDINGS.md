...
本アプリもその条件で分岐する: `needs_xauth` が無いタイルには `return_url` しか
付けず、ブリッジにもトークンを渡さない（公式アプリの挙動と同じ）。
プロフィールページの 4 本はいずれも `true` なので現状は差が出ないが、
povo が求めていないページにトークンを差し出さないための線引き。

`web_view` 以外のアクション種別は、このページには居ない。公式アプリには
`secure_web`（Custom Tab またはボトムシート WebView、トークン注入もブリッジも無し）
という別経路があるが、profile の 4 本はすべて `web_view` なので使わない。
